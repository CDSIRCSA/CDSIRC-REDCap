...
if bdm_list:
    ...
    # Make sure every case has a date of death
    try:
        ...
        for i, row in df.iterrows():
            if row['Date of Death'] == '':
                st.error("ERROR: {GivenNames} {Surname} does not have a date of death.".format(GivenNames=row['Given Names'], Surname=row['Surname']))
        ...
        # Age group
        def age_group(row):
            if row['Date of Birth'] and row['Date of Death']:
